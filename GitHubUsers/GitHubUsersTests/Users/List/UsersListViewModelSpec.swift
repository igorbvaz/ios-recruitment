...
class UsersListViewModelSpec: QuickSpec {
    override func spec() {
        describe("UsersList ViewModel") {
            ...
            describe("its coordinator behavior") {
            ...
            describe("its outputs behavior") {
                ...
                var searchTextObserver: TestableObserver<String>!
                ...
                beforeEach {
                    ...
                    searchTextObserver = scheduler.createObserver(String.self)
                    viewModel.outputs.searchTextDriver.drive(searchTextObserver).disposed(by: disposeBag)
                }
                ...
                context("when users taps cancel") {
                    beforeEach {
                        scheduler.createColdObservable([.next(10, ())]).bind(to: viewModel.inputs.cancelButtonTappedPublishSubject).disposed(by: disposeBag)
                        scheduler.start()
                    }

                    it("should output empty searchText") {
                        expect(searchTextObserver.events.first?.value.element).to(equal(""))
                    }
                }
